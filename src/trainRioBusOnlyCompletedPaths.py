...
best_nn_location = '../data/best_nn_cross_rio_bus_completed.h5'
...
if shouldTrain:
    ...
    for path_id in range(minimum_path, maximum_path, 4):
        ...
        current_train_df = [create_training_path(current_path_df) + [line]]
        train_df = train_df.append(current_train_df)
        if 100 * path_id / total > count_int:
            count_int += 1
            print(count_int)
    train_df.to_csv('../data/' + 'train_df_completed.csv', index=False, header=False)
    exit()

train_df = pd.read_csv('../data/train_df_completed.csv', header=None)
